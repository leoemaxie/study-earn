...
def scrape_scholarships():
    try:
        ...
        for item in soup.select('.item-list-li'):
            image = f"{base_url}{item.find('img')['src']}"
            print(image)
            link = item.select_one('.item-h2 a')['href']
            details = get_scholarship_details(link if link else {image: "No link available"})
            scholarships.append(details.update({'image': image}))
